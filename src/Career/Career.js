import './Career.css';


function Career() {
    return (
        <>
            <div id="career" className='Career_page'>

                <div className="Career">
                    <span className="CHlight">Career</span>
                    <span className="span_border1"></span>

                </div>
                <div className="Cdiv">
                
                    <span className='sAbout' id="best">A career at <strong>xxxxxxxx Hospital</strong> means an opportunity to explore your potential,
                        continuous growth and the joy of working on the latest
                        technologies alongside the best minds in the industry.</span>
                    <span className="sAbout">Please apply with your CV to <strong>hr@xxxxxxxhospital.com</strong>.</span>
                </div>
            </div>


        </>
    );
}
export default Career;